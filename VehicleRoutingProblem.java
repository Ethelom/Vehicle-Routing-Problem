...
public class VehicleRoutingProblem {
    ...
    */
    public VehicleRoutingProblem(int totalServicePoints, int totalTrucks) {
        ...
        VND();
        //TabuSearch();
        //SimulatedAnnealingOperation();
        calculateMaxRoute();
        double time = 0;
        for (int i = 0; i < -1 +maxRoute.getRouteNodes().size(); i++) {
        	time += timeM[maxRoute.getRouteNodes().get(i).getNodeID()][maxRoute.getRouteNodes().get(i+1).getNodeID()]; 
        }
        System.out.println("Max hour : "  +time);
        for (int i = 0; i < -1 +routes.get(24).getRouteNodes().size(); i++) {
        	time += timeM[maxRoute.getRouteNodes().get(i).getNodeID()][maxRoute.getRouteNodes().get(i+1).getNodeID()]; 
        }
        System.out.println("Max hour : "  +time);
        /*while (!endMove) {
    ...
    private void SimulatedAnnealingOperation() {
    ...
    private void TabuSearch() {
    	for (int i = 0; i < 10; i++) { 
            ...
            System.out.println("max route in tabu " + maxRoute.getTotalRouteTimeInHrs());
            calculateMaxRoute();
            System.out.println("i has value" + i);
            if (LocalOptimumHasBeenReached(operatorType, rm, sm, top)) {
            	System.out.println("move cost:" + top.getMoveCost()
            	);
    ...
    private boolean LocalOptimumHasBeenReached(int operatorType, RelocationMove rm, SwapMove sm, TwoOptMove top) {
        if (operatorType == 1) {
        	...
        	System.out.println("compare: " +top.getNewMaxRouteWithThisTwoOptMove()+ " max " +maxRoute.getTotalRouteTimeInHrs());
	...
	private void InitializeOperators(RelocationMove rm, SwapMove sm, TwoOptMove top) {
        ...
        rm.setMaxRouteTime(Double.MAX_VALUE);
        sm.setNewMaxRouteWithThisSwapMove(Double.MAX_VALUE);
                ...
                for (int nodeInd1 = 0; nodeInd1 < rt1.getRouteNodes().size() - 1; nodeInd1++) {
                    ...
                    for (int nodeInd2 = start2; nodeInd2 < rt2.getRouteNodes().size() - 1; nodeInd2++) 
                    {
                        ...
                        if (rt1 == rt2) {
                            ...
                            for (int i = nodeInd1 + 1 ; i < rt2.getRouteNodes().size(); i++)
                            {
    ...
    private void ApplyTwoOptMove(TwoOptMove top) 
    {
        ...
        else
        {
            ...
            rt1.setTotalTimeFromScratch(newTime1);
            rt2.setTotalTimeFromScratch(newTime2);
        }
	...
	private void applyBestRelocationMove(RelocationMove rm) {
        ...
        Route originRoute = routes.get(rm.getOriginRoutePosition());
